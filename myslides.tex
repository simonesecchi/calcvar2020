...
non pu\`o essere soddisfatta. Basta considerare il caso $f=0$ e la
successione $\{n \varphi_k\}_n$, dove $\varphi_k$ \`e un'autofunzione
associata all'autovalore $\lambda_k$. Risulta $J(n \varphi_k)=0$ e
$J'(n \varphi_k)=0$ per ogni $n$, sebbene non esistano
...
$L^{r_0}(\Omega)$ verso $g(\cdot,u)$. Per un risultato fondamentale di
continuit\`a degli operatori di Nemitskii --- che estende quello
dimostrato in questo corso --- $g(\cdot, u_{n_j})$ converge a
$g(\cdot,u)$ in $L^{(p+1)/p}(\Omega)$, dunque a maggior ragione in
$H^{-1}(\Omega)$.
...
Sia pertanto $u \in {\cal N}$ tale che $I(u)=m$. Per ogni $v \in
